...
## 2026-08-19 — AI-development and comparisons reconciliation

Revisited the public development narrative after the v4.0.3 expression and immutable collection work. The AI-development page now describes the actual human-directed agentic workflow in more depth: architectural discussion and challenge before implementation, contract-first checkpoints, autonomous bounded execution, durable repository memory, and multiple agents/models working against shared evidence rather than one-shot “build this” prompting. It names ChatGPT and Codex's repository-level roles and records local Qwen evaluation through OpenCode as another independent workspace-level perspective. The comparisons page was also reconciled with Nift's newer expression, pagination and functional-style collection capabilities, especially where those capabilities move the boundary at which a React-style client framework becomes necessary. Public claims remain architectural comparisons rather than universal superiority claims.
